...
|---|---|---|
| 0.1.1 | `openclaw-v0.1.1` | `@@FES_PIN_SHA@@` (substituted with the tag commit SHA at package build time) |
...
```bash
# must output exactly: openclaw-v0.1.1
git -C "$FES_ROOT" describe --exact-match --tags HEAD

# must match the full commit SHA from the GitHub release notes for openclaw-v0.1.1
git -C "$FES_ROOT" rev-parse HEAD
```
