...
|---|---|---|
| 0.1.6 | `openclaw-v0.1.6` | `a069766b95b00c53329f9df84907b2328cfe7b8c` |
...
```bash
git clone --depth 1 --branch openclaw-v0.1.6 \
...
```bash
# must output exactly: openclaw-v0.1.6
git -C "$FES_ROOT" describe --exact-match --tags HEAD

# must equal the pinned SHA from the table above
if [ "$(git -C "$FES_ROOT" rev-parse HEAD)" != "a069766b95b00c53329f9df84907b2328cfe7b8c" ]; then
  echo "pin mismatch" >&2
  exit 1
fi
```
